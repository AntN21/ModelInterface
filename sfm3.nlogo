...
to setup-regions
  ; setup regions
  ; There is a an id for each type of cell
  ; 0:outside / quai
  ; 1: wall
  ; 2: door
  ; 3: inside / rame de métro
  ; 4: place (pas utilisé)
  ; 5: exit
  let w 3
  let ydoor 0
  ask patches [set region 0 set accessible true]
...
to-report co [r]
  ; return color for type of cell
  let c (list white black red yellow orange pink blue)
  report item r c
end
...
to r-tri-ped  [nb x0 y0 xx yy k r]
  ; set agent in triangles around the door
  ; nb number of agent
  ; x0 y0 "head" of triangle
  ; xx yy delta between agents
  let counter nb
...
to sfm
  ...
  ask peds with [state > -1 ]
    [
      ...
      let angle 0
      if (patch-here != patch targetx targety)[ ; prevent an issue where an agent is already on its target
  ...
  ask peds [ ; Repulsion force of walls ( could be made better)
...
to change-state2
  ; function which changes states i.e. targets
  ; C'est pas très propre, l'idée c'est
  ; - on commence avec la porte comme cible
  ; - on atteint la porte, on choisit une place, on considère qu'elle est occupée, i.e( set accessible false)
  ; - on a atteint notre target on bouge plus trop

  ; aussi le role d'une personne est censé rendre plus simple l'introduction
  ask peds [
    if role = 0 [
      if state = 0 [
        if  outside?  [ ;
...
to-report choose-place [mode] ; "opti" "poisson" "random"
  ; Choose a mode of decision for an agent to choose a place.
  ...
  ;if mode = "normal" [let boo one-of [ 0 1 ]
  ;  report item min (list n (length places))  places]
...
to sfm-init
  ; Does sfm steps with walls around the door to do a sort of initialisation of people around the door.
  tick-advance 1
  change-state2
  let stage -1
...
to move
  ...
  ; sfm does one step of social force model
  ; change-state2 changes states
  ; states are things like i'm outside the train, i'm inside the train, i have arrived;
  ; each state is a new target.
